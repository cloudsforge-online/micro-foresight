 ...
 * outbox — 19 rows across three topics, none of them registered yet:
 *
 *     as shipped -> malformed: version: missing, correlationId: missing
 *     fixed      -> well-formed; only the registration is outstanding
 *
 * `foresight.market.closed` is written by the close sweep with a null correlation id, so a
 * version-only patch would have left it refused anyway. All three fields are mapped here.
 *
 ...
 */
export const MARKET_OPENED = 'foresight.market.opened'
export const MARKET_CLOSED = 'foresight.market.closed'
export const MARKET_RESOLVED = 'foresight.market.resolved'
 ...
 *
 *   const market = await withOutbox(sql, SERVICE, async (tx, emit) => {
 *     const row = await openMarket(tx, input)
 *     emit({ topic: MARKET_OPENED, key: row.id, payload: { id: row.id } })
 *     return row
 *   })
 *
...
export function buildEnvelope(row: OutboxRow): EventEnvelope {
  return {
    ...
    // `system` is the contract's own value for "no principal did this" — a scheduled close sweep, which
